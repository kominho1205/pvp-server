...
app.use((req, res, next) => {
    ...
    res.header('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); // 캐시 비활성화
    res.header('Pragma', 'no-cache'); // HTTP 1.0 캐시 비활성화
    res.header('Expires', '0'); // 캐시 만료
    next();
});
